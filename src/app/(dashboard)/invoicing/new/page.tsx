...
import { addNewInvoice } from "@/lib/actions";
...
export default function NewInvoicePage() {
  ...
  const onSubmit = (values: z.infer<typeof InvoiceSchema>) => {
    startTransition(async () => {
      const result = await addNewInvoice(values);
      if (result.success) {
        toast({ title: "Invoice Created", description: `Invoice ${values.invoiceNumber} has been successfully created.` });
                ...
                <FormItem className="grid gap-2">
                  <FormLabel>Bill To</FormLabel>
                   <Select onValueChange={field.onChange} defaultValue={field.value}>
                        <FormControl><SelectTrigger><SelectValue placeholder="Select or create a customer" /></SelectTrigger></FormControl>
                        <SelectContent>
                          {clients.map(client => (
